...
class CanonicalNode:
    def __init__(
        ...
        if symmetry_plane is None:
            ...
            ).value  # fallback to NONE if not found
        ...
        # self.axis_face = axis
        self._symmetry_plane = symmetry_plane
        self._axial_faces = axial_idx
        self._radial_faces = radial_idx
    ...
    @property
    def children(self):
        for idx, child in enumerate(self.axial_side):
            if child is not None:
                yield (self._axial_faces[idx], child)

        for idx, child in enumerate(self.radial_side):
            if child is not None:
                yield (self._radial_faces[idx], child)
...
if __name__ == "__main__":
    ...
    graph, data_dict = generate_random_individual(
        num_modules, return_other_graph=True,
    )
