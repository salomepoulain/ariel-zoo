...
class CanonicalConfigFactory:
    ...
    Note: Radial faces are ordered such that radial_shift is always 1.
    This simplifies the mental model: one rotation quantum = one position shift.
    """
    ...
    def create(
        ...
        radial_face_order, radial_adjustments = (
            cls._build_radial_configuration(
                radial_faces,
                face_quats,
                rotation_axis,
        ...
        return CanonicalConfig(
            axial_face_order=tuple(axial_face_order),
            radial_face_order=tuple(radial_face_order),
            # radial_shift=1,  # Always 1 by design!
            # symmetry_plane=rotational_symmetry.value,
            unique_rotation_amt=unique_rotation_amt,
            priority=priority.value,
            radial_adjustments=radial_adjustments,
        )
    ...
    def _build_radial_configuration(
    ...
    ) -> tuple[list[ModuleFaces], tuple[int, ...]]:
        """Build radial face configuration with ordering that ensures shift=1.

        Returns:
            (face_order, adjustments)
        """
        if not radial_faces or not face_quats or rotation_axis is None:
            return [], ()

        # Order faces so that after one rotation quantum, they shift by exactly 1 position
        ...
        adjustments = cls._calculate_radial_adjustments(
            ...
            face_quats,
            rotation_axis,
            shift=1,  # Now hardcoded to 1
            unique_rotation_amt=unique_rotation_amt,
        )

        return face_order, adjustments
    ...
    def _order_radial_faces_for_unit_shift(
        ...
        # If natural_shift = k, we need to permute the list so shift becomes 1
        # Solution: Apply the inverse permutation
        # If rotating causes shift k, we want to pre-shift by (1-k) to get final shift of 1
    ...
    def _compute_unique_allowed_rotations(
        allowed_rotations: list[ModuleRotationsIdx],
        symmetry_plane: RotationalSymmetry,
    ) -> int:
