...
class BokehConfig(BaseModel):
    ...
    image_markers: bool = False
    image_marker_size: int = 32
...
class BokehGridPlotter:
    ...
    def _apply_style_cache(self, df: pd.DataFrame) -> None:
        ...
        # Ensure img_html is updated from thumb_cache even if data was added first
        df["img_html"] = self._v_get_html_img(df["global_id"])
    ...
    def _compile_figure(self, cell: CellData, bounds: tuple | None, heatmap_bounds=None):
        ...
        if is_heatmap:
            ...
            if cell.heatmap_ids is not None:
                ...
                ds_data["ids"] = [f"({int(p_val[0])}, {int(p_val[1])})" for p_val in flat_ids]
        ...
        if self.config.image_markers and self.thumb_cache:
            cell.df["url"] = cell.df["global_id"].map(self.thumb_cache)
            src = ColumnDataSource(cell.df)
            sz = self.config.image_marker_size
            p.image_url(
                url="url", x="x", y="y",
                w=sz, h=sz, source=src,
        ...
        src = ColumnDataSource(cell.df)
        p.scatter(
            "x", "y", source=src, color="color", size="size", fill_alpha="alpha",
            line_color=None,
        ...
        if self.config.hover_enabled:
            p.add_tools(HoverTool(tooltips='<div style="padding:0px;">@img_html{safe}<br><b>ID: @global_id</b><br>@tooltip_text</div>'))

        return p, ("scatter", src)


    def apply_jitter(self, amount: float | None = None) -> BokehGridPlotter:
        """
        Applies a random Gaussian offset to scatter plot coordinates to reveal
        overlapping points.
        """
        jitter_val = amount if amount is not None else self.config.jitter_amount

        for cell in self._cells.flatten():
            if cell.cell_type == CellType.SCATTER and not cell.df.empty:
                # Calculate the spread based on the data's bounding box
                x_range = cell.df["x"].max() - cell.df["x"].min()
                y_range = cell.df["y"].max() - cell.df["y"].min()

                # Avoid division by zero for single-point plots
                x_range = x_range if x_range > 0 else 1.0
                y_range = y_range if y_range > 0 else 1.0

                # Apply normal distribution noise
                cell.df["x"] += np.random.normal(0, x_range * jitter_val, size=len(cell.df))
                cell.df["y"] += np.random.normal(0, y_range * jitter_val, size=len(cell.df))
    ...
    def show(self, super_title: str | None = None) -> None:
        ...
        # 2. Heatmap bounds
        heatmap_cells = [c for c in self._cells.flatten() if c.cell_type == CellType.HEATMAP and c.raw_heatmap is not None]
        heatmap_bounds = None
        if heatmap_cells:
        ...
        for r in range(self.n_rows):
            ...
            for c in range(self.n_cols):
                ...
                fig, src_info = self._compile_figure(self._cells[r, c], bounds, heatmap_bounds)
                row_figs.append(fig)
                if src_info:
            ...
            for i, src in enumerate(scatter_sources):
                ...
                sync_callback = CustomJS(
                    ...
                    code="""
                        ...
                        for (let idx of sel_indices) { selected_ids.add(source.data['global_id'][idx]); }
                        for (let target of targets) {
                            const target_indices = [];
                            for (let j = 0; j < target.data['global_id'].length; j++) {
                                if (selected_ids.has(target.data['global_id'][j])) { target_indices.push(j); }
                            }
        ...
        # 5. Link scatter selection to heatmap highlighting
