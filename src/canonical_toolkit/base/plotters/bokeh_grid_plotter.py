...
class BokehGridPlotter:
    ...
    def _compile_figure(self, cell: CellData, x_r, y_r):
        ...
        if self.config.hover_enabled:
            p.add_tools(
                HoverTool(
                    tooltips='<div style="padding:0px;">@img_html{safe}<br><b>ID: @global_id</b></div>',
                ),
            )
    ...
    def show(self, super_title: str | None = None) -> None:
        ...
        if len(scatter_sources) > 1:
            for i in range(1, len(scatter_sources)):
                scatter_sources[0].selected.js_link(
                    "indices",
                    scatter_sources[i].selected,
                    "indices",
                )
                scatter_sources[i].selected.js_link(
                    "indices",
                    scatter_sources[0].selected,
                    "indices",
                )
        ...
        if self.config.sidebar_enabled and sources:
            sources[0].selected.js_on_change(
                "indices",
                CustomJS(
                    args={"s": sources[0], "g": gallery},
                    code="""
                ...
                let h = `<div style='display: flex; gap: 15px;'>`; // Start Flexbox container
                for (let i of idxs) {
