...
from ..node import deriver, node_from_graph
from .options import (
    MatrixDomain,
    OutputType,
...
def collect_population_fingerprint(
    ...
    *,
    ...
    return [
        collect_hash_fingerprint(node, config=config)
        for node in node_population
    ]
...
def series_from_population_fingerprint(
    ...
    *,
    ...
    if max_radius is None:
        max_radius = max(
            (max(d.keys()) for d in population_fingerprint if d), default=0
        )
    ...
    for radius in range(max_radius + 1):
        ...
        processed_features = [feat or fill for feat in radius_features]
...
def series_from_graph_population(
    ...
    return series_from_population_fingerprint(
...
def frame_from_graph_population(
    ...
    for config in space_configs:
        fingerprint = [
            collect_hash_fingerprint(n, config=config) for n in node_population
        ]
