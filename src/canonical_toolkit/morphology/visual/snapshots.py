...
def _default_snapshot_converter[T](data: T) -> Image.Image:
...
def _save_single_image_worker(args):
    ...
    try:
        ...
        console.print(f"[dim]{format_exc()}[/dim]")
        return None
...
def snapshot_saver(
    ...
    *,
    ...
    if len(snapshot_naming) != len(raw_data):
        msg = f"snapshot_naming length ({len(snapshot_naming)}) must match raw_data length ({len(raw_data)})"
    ...
    if use_parallel:
        ...
        try:
            ...
            console.print(
                f"[green]Saved {len(successful)}/{len(raw_data)} snapshots to {save_folder}[/green]"
            )
        except Exception as e:
            console.print(
                f"[yellow]Multiprocessing failed ({e}), falling back to serial...[/yellow]"
            )
            use_parallel = False
    ...
    if not use_parallel:
        ...
        for arg in track(args, description="Saving snapshots (serial)..."):
            ...
            if result is not None:
                successful += 1
        console.print(
            f"[green]Saved {successful}/{len(raw_data)} snapshots to {save_folder}[/green]"
        )
...
def snapshot_loader(
    ...
    if not snapshot_folder.exists():
        msg = f"Image folder not found: {snapshot_folder}"
    ...
    if snapshot_names is None:
        ...
        if not snapshot_names:
            console.print(
                f"[yellow]No .png files found in {snapshot_folder}[/yellow]"
            )
            return []
    ...
    for name in track(
        snapshot_names,
        description=f"Loading {len(snapshot_names)} snapshots...",
    ):
        snapshot_path = snapshot_folder / name
        if not snapshot_path.exists():
            console.print(
                f"[yellow]Warning: {name} not found, skipping[/yellow]"
            )
            continue
    ...
    *,
    ...
    if use_parallel:
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as executor:
            ...
            futures = [
                executor.submit(_load_and_encode_thumbnail, arg) for arg in args
            ]
            thumbnails.extend(
                future.result()
                for future in track(
        ...
        for name in track(
            ...
            thumbnails.append(
                _load_and_encode_thumbnail((name, snapshot_folder, scale))
            )
